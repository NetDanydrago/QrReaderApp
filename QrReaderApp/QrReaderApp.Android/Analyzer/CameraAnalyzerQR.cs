...
{
	public class CameraAnalyzerQR
	{
		readonly CameraControllerQR cameraController;
		readonly CameraEventsListenerQR cameraEventListener;
		Task processingTask;
		...
		public CameraAnalyzerQR(PageRenderer pageRender,SurfaceTexture surface, IScannerSessionHost scannerHost)
		{
			this.scannerHost = scannerHost;
			cameraEventListener = new CameraEventsListenerQR();
			cameraController = new CameraControllerQR(pageRender,surface, cameraEventListener, scannerHost);
			Torch = new TorchQR(cameraController, pageRender.Context);
	
		}

		public Action<Result> BarcodeFound;

		public TorchQR Torch { get; }
